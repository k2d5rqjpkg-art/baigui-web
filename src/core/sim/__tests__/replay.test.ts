...
describe('Day42: Recorder + replay', () => {
  it('录制 N 步 → 同 seed 重放 → tick 完全一致', () => {
    ...
    for (let i = 0; i < 30; i++) {
      const a: Action[] = [{ type: 'move', entityId: 'e_p1' as EntityId, payload: { dx: 1, dy: 0 } }];
  ...
  it('录制帧保留 playersHp', () => {
    ...
    const rec = new Recorder(seed);
    let state = makeWorld(seed);
    const a: Action[] = [{ type: 'attack', entityId: 'e_p1' as EntityId, payload: { targetId: 'e_m1' as EntityId } }];
    const r = tick(state, a, 50, { layout });
    rec.record(state, a, r.events.length);
