...
| 维度 | 数值 |
|---|---|
| **代码** | **13,797 行** (src/ + server/, 排除测试) |
| **测试** | **8,439 行** (61 文件) |
| **TypeScript 源文件** | 126 个 |
| **测试** | **561 通过 / 1 跳过 (562)** |
| **覆盖率** | **76.88%** (全文件) |
| **CI 时长** | ~17s (typecheck+lint+test+build) |
| **Git commits** | 57 |
| **node_modules** | 283 MB |
| **dist/** | 667 KB |
...
| `server/` (服务) | 2,339 | bridge 10% / state 75% |
...
| **5+10 raid 副本** | `DungeonConfig` + `distributeLoot` (participants 列表) | ✅ Day13 |
...
| `server/bridge.ts` | 10% | 15% | 10% | 10% (HTTP 入口) |
...
| 类型安全 | "模块呈非结构化特征" | **TS strict, 0 any (目标)** |
...
- Lint 仍有些 `any` warning (覆盖测试里的 mock 数据)
