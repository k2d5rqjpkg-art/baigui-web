...
function seedWorld(): { state: GameState; layout: ReturnType<typeof worldGen> } {
  ...
  state = addEntity(state, {
    ...
    kind: 'player',
    pos: { x: 10, y: 10 },
    hp: 100, maxHp: 100, atk: 30, def: 5, level: 3,
  ...
  state = addEntity(state, {
    ...
    kind: 'monster',
    pos: { x: 11, y: 10 },
    hp: 30, maxHp: 30, atk: 10, def: 1, level: 1,
...
const actionArb = (eid: EntityId, tid: EntityId): fc.Arbitrary<Action> =>
  fc.oneof(
    fc.tuple(fc.constant('move'), fc.integer({ min: -1, max: 1 }), fc.integer({ min: -1, max: 1 })),
    ...
    if (k === 'attack') return { type: 'attack', entityId: eid, payload: { targetId: x } } as Action;
...
describe('Day41: fuzz sim tick 100 轮', () => {
  ...
  it('大量随机动作 (50-150 步) 后 state 仍结构合法', () => {
    fc.assert(
      fc.property(fc.array(actionArb('e_p1' as EntityId, 'e_m1' as EntityId), { minLength: 50, maxLength: 150 }), (batch) => {
